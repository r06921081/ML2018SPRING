...
pics_flat = np.reshape(pics, (pic_num, -1))
avgface = np.sum(pics_flat, axis=0)/pic_num
# plt.figure()
# plt.imshow(avgface.reshape(size,size,3))
# plt.show()
pic_center = pics_flat - avgface # x - u
...
# fig = plt.figure()
for imgNo, i in zip([0,100,200,300], list(range(4))):
    imgs = pics[imgNo]
