...
def inputdata():
  ...
  listdata = []
  c = np.zeros((1, 19), dtype=np.float64)  
  for timeline, subdata in rawdata.items():
    temp = np.zeros((1, 19), dtype=np.float64) 
    temp[0][0] = timeline
    temp = placewell(temp, subdata)
    # print(temp)
    c = np.vstack((c,temp)) 
...
def mgradientDescent(X, y, theta, alpha, num_iters):
    ...
    for iter in range(1, num_iters + 1):
      ...
      # gra = np.dot(X_t,loss)
      # print(np.array([gra[0][0],gra[1][0]],dtype=np.float))
      # print(s_gra)
      # s_gra += np.array([gra[0][0],gra[1][0]],dtype=np.float)**2
      # ada = np.sqrt(s_gra)
      # theta = theta - 10 * gra/ada
        if iter % 100 == 0 :
...
if __name__ == '__main__':
    ...
    mtheta = mgradientDescent(np.matrix(moneX), np.matrix(my), np.matrix(mtheta), alpha, iterations)
    ...
    print('-*--------')
    ans = []
    sol = np.dot(np.array(test_x[0],dtype=np.float),w)[0,0]#/10*l
    for i in range(1,len(test_x)):      
      if i % 9 == 0:
        ans.append(["id_"+str(i//9-1)])
        ans[len(ans)-1].append(sol)
        sol = np.dot(np.array(test_x[i+1],dtype=np.float),w)[0,0]#/10*l
      sol = sol * 0.3 + 0.7 * np.dot(np.array(test_x[i],dtype=np.float),w)[0,0]#/10*l
